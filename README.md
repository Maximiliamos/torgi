...
В web-карте iframe и экземпляр Яндекс.Карт создаются один раз: выбор лота,
«светофор», фильтры и кадастровые слои обновляются командами без сброса центра и
масштаба. Лоты с одинаковыми координатами раскрываются в отдельный список, а в
карточке основной метки видны все объединённые публикации с индивидуальными
ссылками; администратор может отделить ошибочно присоединённую публикацию.
...
- Успешные результаты массового GEO кэшируются в PostgreSQL на 30 дней; одинаковый запрос внутри пакета выполняется один раз.
- Массовое GEO использует ограниченный параллелизм (`GEO_MAX_WORKERS=6`, `GEO_NSPD_CONCURRENCY=2` по умолчанию), сначала проверяет НСПД и локальный Photon и добавляет готовые точки на карту сразу после записи в БД.
- Медленные внешние fallback-провайдеры IK12 и Nominatim для bulk-прохода выключены по умолчанию (`GEO_BULK_IK12_FALLBACK=false`, `GEO_BULK_NOMINATIM_FALLBACK=false`), но остаются доступны для интерактивного поиска и могут быть явно включены.
- `GET /api/operations/progress` возвращает безопасные счётчики поиска и геокодирования для индикатора на карте; адреса и данные лотов в ответ не входят.
