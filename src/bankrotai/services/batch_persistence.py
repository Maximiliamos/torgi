...
def _source_values(
    ...
    *,
    ...
    return {
        ...
        "description": lot.description,
        "category": lot.category,
        "region_code": normalize_region_code(str(raw.get("region_code") or lot.region_name or lot.region_slug or "")),
        "region_name": lot.region_name,
        "address": lot.address,
        "cadastral_number": lot.cadastral_number,
        "start_price": _to_decimal(lot.start_price),
        ...
        "efresb_message_number": lot.efresb_message_number
        or _raw_value(raw, "efresb_message_number", "fedresurs_message_number"),
        ...
        "bankruptcy_case_number": lot.bankruptcy_case_number
        or _raw_value(raw, "bankruptcy_case_number", "case_number"),
        "deposit_amount": _to_decimal(
            lot.deposit_amount
            if lot.deposit_amount is not None
            else _to_float(_raw_value(raw, "deposit", "deposit_amount"))
        ),
        "deposit_percent": lot.deposit_percent or _to_float(_raw_value(raw, "deposit_percent")),
        "deposit_payment_details": lot.deposit_payment_details
        or _raw_value(raw, "deposit_payment_details", "deposit_requisites"),
        ...
        or _to_datetime(_raw_value(raw, "bidd_end_time", "application_deadline")),
        ...
        "next_price_reduction_at": lot.next_price_reduction_at
        or _to_datetime(_raw_value(raw, "next_price_reduction_at")),
...
def _persist_changed_lots_chunk(
    ...
    *,
    ...
    session.execute(
        canonical_statement.on_conflict_do_update(
    ...
    old_status_by_id = {external_id: row.auction_status for external_id, row in existing_processed.items()}
    old_price_by_id = {external_id: row.current_price for external_id, row in existing_processed.items()}
    protected_ids = {external_id for external_id, row in existing_processed.items() if row.review_status is not None}
    ...
    geo_input_changed = and_(
        ProcessedLot.review_status.is_(None),
        or_(
            ProcessedLot.address.is_distinct_from(processed_excluded.address),
            ProcessedLot.cadastral_number.is_distinct_from(processed_excluded.cadastral_number),
            ProcessedLot.title.is_distinct_from(processed_excluded.title),
            ProcessedLot.description.is_distinct_from(processed_excluded.description),
            ProcessedLot.region_name.is_distinct_from(processed_excluded.region_name),
        ),
    )
    processed_updates["needs_geo_check"] = case(
        (geo_input_changed, True),
        else_=ProcessedLot.needs_geo_check,
    )
    processed_updates["geo_input_hash"] = case(
        (geo_input_changed, None),
        else_=ProcessedLot.geo_input_hash,
    )
    session.execute(
        processed_statement.on_conflict_do_update(
