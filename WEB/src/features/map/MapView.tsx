...
function directKey(version,z,x,y){return String(version)+':'+z+'/'+x+'/'+y;}
function directTouch(key,payload){directCache.delete(key);directCache.set(key,payload);while(directCache.size>DIRECT_CACHE_MAX){const oldest=directCache.keys().next().value;if(oldest===undefined)break;directCache.delete(oldest);}}
function directTileXY(lat,lon,z){const scale=2**z;const x=Math.max(0,Math.min(scale-1,Math.floor((lon+180)/360*scale)));const bounded=Math.max(-85.05112878,Math.min(85.05112878,lat));const rad=bounded*Math.PI/180;const y=Math.max(0,Math.min(scale-1,Math.floor((1-Math.asinh(Math.tan(rad))/Math.PI)/2*scale)));return{x,y};}
function directVisibleTiles(){if(!map)return[];const z=Math.min(14,Math.max(0,Math.floor(map.getZoom())));const scale=2**z;const b=map.getBounds();const west=b[0][1],south=b[0][0],east=b[1][1],north=b[1][0];const nw=directTileXY(north,west,z),se=directTileXY(south,east,z);const ranges=west<=east?[[nw.x,se.x]]:[[nw.x,scale-1],[0,se.x]];const out=[];for(const range of ranges){for(let x=range[0];x<=range[1];x+=1){for(let y=nw.y;y<=se.y;y+=1)out.push({z,x,y});}}return out;}
function directNeighbors(tiles){if(!tiles.length)return[];const z=tiles[0].z,scale=2**z,existing=new Set(tiles.map(t=>t.z+'/'+t.x+'/'+t.y)),result=new Map();for(const tile of tiles){for(let dx=-1;dx<=1;dx+=1){for(let dy=-1;dy<=1;dy+=1){if(dx===0&&dy===0)continue;const x=(tile.x+dx+scale)%scale,y=Math.max(0,Math.min(scale-1,tile.y+dy)),k=z+'/'+x+'/'+y;if(!existing.has(k))result.set(k,{z,x,y});}}}return[...result.values()];}
function directPreset(status,current){if(['closed','completed','cancelled','canceled','failed','annulled','archive','archived'].includes(String(current||'').toLowerCase()))return'islands#blackDotIcon';return status==='approved'?'islands#greenDotIcon':status==='maybe'?'islands#yellowDotIcon':status==='rejected'?'islands#redDotIcon':'islands#grayDotIcon';}
function registerDirectTile(key,payload){if(!tileManager||tileObjects.has(key)||!payload||payload.type!=='FeatureCollection'||!Array.isArray(payload.features))return;tileManager.add(payload);const ids=[];for(const object of payload.features){ids.push(object.id);if(object.properties?.kind==='lot'){const id=Number(object.properties.lotId??object.id);tileLots.set(id,{id,...object.properties});}}tileObjects.set(key,ids);}
function emitDirectRendered(started){requestAnimationFrame(()=>send('bankrotai-rendered',{durationMs:performance.now()-started,count:[...tileObjects.values()].reduce((n,ids)=>n+ids.length,0)}));}
function requestDirectTile(tile,generation,prefetch=false){if(!directDataset)return;const key=directKey(directDataset.version,tile.z,tile.x,tile.y);const cached=directCache.get(key);if(cached){directTouch(key,cached);if(!prefetch&&generation===directGeneration&&directWanted.has(key))registerDirectTile(key,cached);return;}for(const value of directPending.values()){if(value.key===key){if(!prefetch){value.prefetch=false;value.generation=generation;}return;}}const requestId=String(++directSequence);directPending.set(requestId,{key,generation,prefetch});send('bankrotai-tile-request',{requestId,version:directDataset.version,z:tile.z,x:tile.x,y:tile.y,prefetch});}
function handleDirectResponse(data){const requestId=String(data.requestId||''),entry=directPending.get(requestId);if(!entry)return;directPending.delete(requestId);if(data.error||!data.payload)return;directTouch(entry.key,data.payload);if(!entry.prefetch&&entry.generation===directGeneration&&directWanted.has(entry.key)){const started=performance.now();registerDirectTile(entry.key,data.payload);emitDirectRendered(started);}}
function syncDirectTiles(){if(!map||!tileManager||!directDataset||mode!=='direct')return;const started=performance.now(),generation=++directGeneration,tiles=directVisibleTiles();directWanted=new Set(tiles.map(t=>directKey(directDataset.version,t.z,t.x,t.y)));for(const[key,ids]of tileObjects){if(!directWanted.has(key)){tileManager.remove(ids);ids.forEach(id=>tileLots.delete(Number(id)));tileObjects.delete(key);}}for(const tile of tiles)requestDirectTile(tile,generation,false);emitDirectRendered(started);for(const tile of directNeighbors(tiles).slice(0,12))requestDirectTile(tile,generation,true);}
function setDirectDataset(value){if(!value?.version)return;const changed=directDataset?.version!==value.version;directDataset=value;if(changed){directGeneration+=1;directCache.clear();directPending.clear();tileManager?.removeAll();tileObjects.clear();tileLots.clear();}for(const item of Array.isArray(value.bootstrap_tiles)?value.bootstrap_tiles:[]){if(item?.payload)directTouch(directKey(value.version,item.z,item.x,item.y),item.payload);}mode='direct';activateManager(tileManager);syncDirectTiles();}
function disableDirectTiles(){if(mode!=='direct')return;directGeneration+=1;directDataset=null;directPending.clear();tileManager?.removeAll();tileObjects.clear();tileLots.clear();mode='legacy';renderLots();}
function scheduleDirectTiles(){if(mode!=='direct')return;clearTimeout(directTimer);directTimer=setTimeout(syncDirectTiles,70);}
function updateTileReview(lotId,status){if(!tileManager)return false;const id=Number(lotId),lot=tileLots.get(id);if(!lot)return false;const updated={...lot,review_status:status};tileLots.set(id,updated);tileManager.objects.setObjectOptions(id,mode==='direct'?{preset:directPreset(status,updated.status)}:opts(updated));return true;}
