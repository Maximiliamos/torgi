...
const channel=${safeScriptJson(channel)};const instanceId=(crypto.randomUUID?crypto.randomUUID():String(Date.now())+Math.random());let map=null;let manager=null;let legacyManager=null;let tileManager=null;let lots=[];let mode='legacy';const tileObjects=new Map();const tileLots=new Map();let directEnabled=false;let directDataset=null;let directGeneration=0;let directWanted=new Set();let directTimer=null;let cad=null;let selectedGeometry=null;let showCad=true;let selectedId=null;let pending=[];let overlayObjects=[];let viewportTimer=null;
...
function directPreset(status){return status==='approved'?'islands#greenDotIcon':status==='maybe'?'islands#yellowDotIcon':status==='rejected'?'islands#redDotIcon':'islands#grayDotIcon';}
