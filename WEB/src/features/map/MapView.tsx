...
const channel=${safeScriptJson(channel)};const instanceId=(crypto.randomUUID?crypto.randomUUID():String(Date.now())+Math.random());let map=null;let manager=null;let legacyManager=null;let tileManager=null;let lots=[];let mode='legacy';const tileObjects=new Map();const tileLots=new Map();const tileMutationStats={add:0,remove:0,removeAll:0,setObjectOptions:0};let cad=null;let selectedGeometry=null;let showCad=true;let selectedId=null;let pending=[];let overlayObjects=[];let viewportTimer=null;
...
function syncTiles(entries){if(!map||!tileManager)return;const started=performance.now();if(!entries.length){if(mode==='tiles'){tileManager.removeAll();tileMutationStats.removeAll+=1;tileObjects.clear();tileLots.clear();mode='legacy';renderLots();}return;}if(mode!=='tiles'){activateManager(tileManager);tileManager.removeAll();tileMutationStats.removeAll+=1;tileObjects.clear();tileLots.clear();mode='tiles';}const wanted=new Set(entries.map(entry=>entry.key));for(const[key,ids]of tileObjects){if(!wanted.has(key)){tileManager.remove(ids);tileMutationStats.remove+=ids.length;ids.forEach(id=>tileLots.delete(Number(id)));tileObjects.delete(key);}}for(const entry of entries){if(tileObjects.has(entry.key))continue;const raw=entry.features||[];const features=raw.map(feature);tileManager.add({type:'FeatureCollection',features});tileMutationStats.add+=features.length;raw.forEach(item=>{if(item.kind==='lot')tileLots.set(Number(item.id),item);});tileObjects.set(entry.key,features.map(item=>item.id));}requestAnimationFrame(()=>send('bankrotai-rendered',{durationMs:performance.now()-started,count:[...tileObjects.values()].reduce((n,ids)=>n+ids.length,0)}));}
function updateTileReview(lotId,status){if(!tileManager)return false;const id=Number(lotId),lot=tileLots.get(id);if(!lot)return false;const updated={...lot,review_status:status};tileLots.set(id,updated);tileManager.objects.setObjectOptions(id,opts(updated));tileMutationStats.setObjectOptions+=1;return true;}
