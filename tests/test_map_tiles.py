...
def test_legacy_tile_api_remains_private_while_yandex_tiles_are_public_immutable(monkeypatch):
    ...
    assert s3_current.json()["bundle_index_base_url"] is None

    monkeypatch.setattr(api.settings, "map_object_store_layout", "regional-bundles-v1")
    ...
    assert bundled_payload["bundle_index_base_url"] == (
        f"https://map.example.test/public/datasets/{s3_version}/indexes"
    )
